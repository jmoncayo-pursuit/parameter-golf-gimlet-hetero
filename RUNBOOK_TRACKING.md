...
**Legend:** **Pass** = feasibility check passed · **—** = not run / N/A. **`n/m` in the staging column** = `n` rows in `progress.csv` with `status=OK` out of `m` jobs that notebook plans (each job is one train or one eval slice, depending on runbook). **Good** = `n=m` and no `FAIL_*` rows for planned work.
...
| Experiment | Notebook · repo | Colab done? | All staging runs OK? | Steps match tag (A) | Loss ok longer runs (A) | `ckpt_bytes` | Aggregate signal | Mini val | Runpod ready | Runpod result |
|:---|:---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---|
| Gimlet hetero | `colab_gimlet_runbook.ipynb` · **this repo** | Yes | Pass · 7/7 | Pass | Pass | stable (120728183 B) | `BASELINE_CHECKPOINT_SIGNAL_STRONG: YES` | ~2.49 bpb | Yes | |
