"""
Gimlet Heterogeneous Precision Width Decoder (parameter-golf-gimlet-hetero).

Based on the official OpenAI parameter-golf baseline train_gpt.py.
Changes from baseline are marked with [HETERO] comments.

Layer mapping (11 layers, 0-indexed):
  Early  (0-2):  mlp_mult=3.0, export clip_val=31 (Int6)
  Middle (3-7):  mlp_mult=4.5, export clip_val=7  (Int4)
  Late   (8-10): mlp_mult=3.0, export clip_val=31 (Int6)
  All attention projections: export clip_val=31 (Int6)
...
# HYPERPARAMETERS
# -----------------------------
# [HETERO] Heterogeneous baseline:
# - 11 transformer blocks at width 512 with per-layer MLP width
...
class Hyperparameters:
    ...
    num_layers = int(os.environ.get("NUM_LAYERS", 11))  # [HETERO] 11 for 3/5/3 split
    num_kv_heads = int(os.environ.get("NUM_KV_HEADS", 4))
    model_dim = int(os.environ.get("MODEL_DIM", 512))
    num_heads = int(os.environ.get("NUM_HEADS", 8))
    mlp_mult = float(os.environ.get("MLP_MULT", 4))  # [HETERO] unused, overridden per-layer
...
# -----------------------------
# [HETERO] Per-layer precision and width allocation.
# Early/late layers: narrow MLP (3x), Int6 export.
# Middle layers: wide MLP (4.5x), Int4 export.

EARLY_LAYER_COUNT = 3
LATE_LAYER_COUNT = 3

def get_layer_config(layer_idx: int, num_layers: int) -> tuple[float, int]:
    """Returns (mlp_mult, export_clip_val) for a given layer index.

    clip_val=31 maps to ~Int6 (63 levels), clip_val=7 maps to ~Int4 (15 levels).
    """
    if layer_idx < EARLY_LAYER_COUNT:
        return (3.0, 31)
    elif layer_idx >= num_layers - LATE_LAYER_COUNT:
        return (3.0, 31)
    else:
        return (4.5, 7)
...
def quantize_float_tensor(t: Tensor, clip_val: int = 127) -> tuple[Tensor, Tensor]:
    # [HETERO] clip_val controls effective precision: 127=Int8, 31=Int6, 7=Int4
    t32 = t.float()
    if t32.ndim == 2:
...
def quantize_state_dict_int8(state_dict: dict[str, Tensor], clip_val_map: dict[str, int] | None = None):
    # [HETERO] clip_val_map overrides the default int8 clip range per tensor name.
    # Single supported clean-script export format:
    # - per-row quantization for 2D float tensors (clip range from clip_val_map or 127)
    ...
    for name, tensor in state_dict.items():
        ...
        # [HETERO] Look up per-tensor clip_val; default to 127 (standard int8).
        clip_val = 127
        if clip_val_map is not None and name in clip_val_map:
            clip_val = clip_val_map[name]

        stats["num_float_tensors"] += 1
        q, s = quantize_float_tensor(t, clip_val=clip_val)
        if s.ndim > 0:
    ...
    obj: dict[str, object] = {
        "__quant_format__": "int8_hetero_per_row_v1",  # [HETERO] format identifier
        "quantized": quantized,
        "scales": scales,
        "dtypes": dtypes,
    ...
    # [HETERO] Store the clip_val_map for traceability.
    if clip_val_map:
        obj["hetero_clip_vals"] = {k: v for k, v in clip_val_map.items() if v != 127}
    return obj, stats
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # [HETERO] GQA Compatibility: standard SDPA does not support enable_gqa.
        # We manually repeat K and V heads to match Q.
...
class MLP(nn.Module):
    ...
    def __init__(self, dim: int, mlp_mult: float):  # [HETERO] float for per-layer width
        super().__init__()
        hidden = int(mlp_mult * dim)  # [HETERO] int() for float mlp_mult
...
class Block(nn.Module):
    def __init__(
        ...
        num_heads: int,
        num_kv_heads: int,
        mlp_mult: float,  # [HETERO] float for per-layer width
        rope_base: float,
        qk_gain_init: float,
    ):
        super().__init__()
...
class GPT(nn.Module):
    def __init__(
        ...
        rope_base: float,
        qk_gain_init: float,
    ):  # [HETERO] mlp_mult removed from signature; per-layer via get_layer_config
        super().__init__()
        ...
        # [HETERO] Per-layer block construction with heterogeneous MLP width.
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    num_heads,
                    num_kv_heads,
                    get_layer_config(i, num_layers)[0],  # [HETERO] per-layer mlp_mult
                    rope_base,
                    qk_gain_init,
                )
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        # [HETERO] mlp_mult removed; per-layer via get_layer_config inside GPT
    ...
    for i in range(args.num_layers):
        _, export_clip = get_layer_config(i, args.num_layers)
        for suffix in ("mlp.fc.weight", "mlp.proj.weight"):
            clip_val_map[f"blocks.{i}.{suffix}"] = export_clip
        for suffix in ("attn.c_q.weight", "attn.c_k.weight", "attn.c_v.weight", "attn.proj.weight"):
            clip_val_map[f"blocks.{i}.{suffix}"] = 31  # All attention stays Int6
    ...
    for i, block in enumerate(base_model.blocks):
        is_middle = EARLY_LAYER_COUNT <= i < args.num_layers - LATE_LAYER_COUNT
    ...
    optimizer_muon = Muon(
        middle_mlp_params,  # [HETERO] Muon on middle MLP matrices only
        lr=args.matrix_lr,
    ...
    # [HETERO] Log heterogeneous layer config and optimizer group sizes.
    for i in range(args.num_layers):
        mlp_m, clip_v = get_layer_config(i, args.num_layers)
        group = "early" if i < EARLY_LAYER_COUNT else ("late" if i >= args.num_layers - LATE_LAYER_COUNT else "middle")
